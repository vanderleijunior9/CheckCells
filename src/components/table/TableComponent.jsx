...
const TableComponent = () => {
  return (
    <div className="p-4 bg-washed-whitewhite border rounded-lg shadow-xl">
      <div className="flex items mb-4">
        <div className="relative flex ">
          <input
            type="text"
            placeholder="Value"
            className="w-full pl-3 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-black text-black "
          />
          <div className="absolute right-3 top-1/2 -translate-y-1/2">
            <Search size={16} className="text-gray-500" />
          </div>
        </div>
        <select className=" ml-170  py-1 p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-black text-slate-500">
          <option className="text-black">Pick an option</option>
          {/* Add more later */}
        </select>
      </div>
      ...
      <table className="w-full border-collapse text-black rounded-md">
        ...
        <tr className="bg-washed-grey">
          <th className="py-2 px-4 text-left rounded-2xl bg-washed-grey">Diagnostician Name</th>
  ...
  {data.map((row, index) => (
    <tr key={index} className="even:bg-gray-50">
      <td className="py-2 px-4">{row.diagnosticianName}</td>
      <td className="py-2 px-4">{row.testId}</td>
      <td className="py-2 px-4">{row.dateOfTest}</td>
      <td className="py-2 px-4">{row.testType}</td>
